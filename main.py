...
ENABLE_OPENING_RESERVE_V6 = os.environ.get("ORBIT_WARS_ENABLE_OPENING_RESERVE_V6", "0") != "0"
...
ATTACK_VALUE_HORIZON = float(os.environ.get("ORBIT_WARS_ATTACK_VALUE_HORIZON", "135.0"))
ATTACK_TRAVEL_PENALTY = float(os.environ.get("ORBIT_WARS_ATTACK_TRAVEL_PENALTY", "1.1"))
OPENING_ATTACK_TRAVEL_LIMIT = float(os.environ.get("ORBIT_WARS_OPENING_ATTACK_TRAVEL_LIMIT", "22.0"))
...
def defense_reserve(planet, step):
    ...
    if step < 90 and planet.production <= 1:
        ...
        return min(5, max(2, int(planet.ships * 0.25)))
...
def attack_plan_for_wait(src, target, planets, fleets, player, step, initial_by_id, angular_velocity, wait_turns):
    ...
    if strongest_contester >= ships_needed and target.owner == -1:
...
def target_score(src, target, planets, fleets, player, step, initial_by_id, angular_velocity, comet_ids):
    ...
    if step < 80 and src.production <= 2 and target.owner == -1 and weak_opening_target:
        ...
        if affordable_soon and target.id != anchor["id"] and src.ships < anchor["ships"]:
            return None
    ...
    plans = [
        ...
        for plan in [attack_plan_for_wait(
            src, target, planets, fleets, player, step, initial_by_id, angular_velocity, wait_turns
        )]
        if plan is not None
    ]
    if not plans:
...
def build_attack_candidates(my_planets, planets, fleets, player, step, initial_by_id, angular_velocity, comet_ids):
    candidates = []
    for src in my_planets:
        for target in planets:
            ...
            candidate = target_score(
                src, target, planets, fleets, player, step, initial_by_id, angular_velocity, comet_ids
            )
...
def choose_moves(candidates, committed, step, max_moves, min_score=12.0, source_pressure=None, threat_reserve=None):
...
def agent(obs):
    ...
    attacks = build_attack_candidates(
        my_planets, planets, fleets, player, step, initial_by_id, angular_velocity, comet_ids
    )
    ...
    if DEBUG and moves:
        ...
        print(f"step={step} player={player} " + " | ".join(notes), file=sys.stderr)
