"""Train/eval/export parity for GARRISON-ranked source selection (>MAX_OWNED owned).

When >MAX_OWNED planets are owned, the source slots are the highest-garrison owned planets
...
tensor order. Covers >MAX_OWNED owned + an exact garrison tie (the index-tiebreak path).
...
def _board(n_owned=30):
    ...
    env.fleets[:] = 0
    # Distinct ids, garrisons designed so the top-MAX_OWNED set + order is unambiguous, plus ONE tie
    # (two planets at 50 ships) to exercise the lowest-index tiebreak.
    ships = [
        5, 90, 12, 50, 7, 200, 33, 50, 8, 120,
        3, 77, 19, 140, 2, 66, 41, 9, 150, 4,
        58, 23, 175, 11, 36, 82, 6, 101, 27, 14,
    ]
...
def test_source_selection_parity():
    ...
    assert len(pids_torch) == MAX_OWNED, f"expected {MAX_OWNED} valid slots, got {len(pids_torch)}"
    assert pids_torch == pids_feat == pids_mask, (
        f"source-selection parity MISMATCH:\n torch={pids_torch}\n feat ={pids_feat}\n mask ={pids_mask}")

    # Ground truth: top-MAX_OWNED by (-round(ships), array index). ids are 100+i.
    expected = [100 + i for i, _ in sorted(enumerate(ships), key=lambda t: (-t[1], t[0]))[:MAX_OWNED]]
    assert pids_torch == expected, f"not top-{MAX_OWNED}-by-garrison:\n got={pids_torch}\n exp={expected}"
    ...
    print(f"ok: train/eval/export agree on top-{MAX_OWNED}-by-garrison (30 owned). slots={pids_torch}")
...
def test_no_op_under_cap():
    ...
    print(f"ok: <=MAX_OWNED ({MAX_OWNED}) owned — all owned planets slotted, paths agree")
