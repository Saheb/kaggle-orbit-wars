...
arrival deadline `D` anchored in observable state, the reactive `floor(D)`, how much friendly
...
reinforce_cost(P)   = remaining(P)            # ships to fill the hold deficit; HOLDABLE iff hold_value ≥ this
min_material_ships  = 3        # absolute floor on a "material" inbound group (with material_frac)
...
```
`tol` interval semantics (do not collapse these to one inequality):
```
ready_now(src)              : abs(slack_to_anchor) <= tol
inbound_counts_for_floor(f) : arrival_eta <= D + tol
...
ready_now(s, D)  = D_abs in arrival_range       # the source can land in [D-tol, D+tol] at SOME legal count

# size = max(share, on-time) capped at capacity — resolves the quota>safe case:
n_ontime      = smallest legal bin with current_step + ETA(n) <= D_abs + tol     # timing lower bound (more=faster)
n_want        = max( quota , n_ontime )                                          # share AND on-time
n_cap         = largest legal bin <= safe_sendable                               # capacity
chosen_bin    = min( n_want , n_cap )
viable(s)     = chosen_bin satisfies current_step + ETA(chosen_bin) in [D-tol, D+tol]
               # if n_ontime > n_cap (even full safe arrives too late) → NOT viable this step (defer / it's too far)
               # if quota > n_cap (full safe < share) → send full safe; wave under-fills THIS step, the
...
post_round    : assert viable(s); Σ_ready chosen_bin recorded vs (floor − cover)   # parity-critical; log misses
```

`choose_anchor`, the floor's inbound/cover windows, and `slack_to_anchor` all use this same
actual-count ETA. Features expose `eta_fast`/`eta_slow` so the achievable arrival range is
visible. This subsumes the rounding rule (§6) — rounding is "smallest bin ≥ quota that lands
on time," not a free nearest-bin choice.
...
```
`reactive_enemy_mass_to_T(D)` is computed from **current enemy planet garrisons** (the
ETA-binned feature, cumsum over bins with reach-ETA ≤ D). Launched enemy fleets have already
left their garrison, so committed-elsewhere mass is excluded automatically (see §3.2).

### 3.2 classify_enemy_fleet(f, T, D)  — exactly one of three
```
f → T,  arrives <= D+tol   ⇒ raises capture_floor(T)       # it will be defending T when we land
f → T,  arrives  > D+tol   ⇒ recapture_risk(T)             # post-capture hold problem, NOT capture floor
...
ATTACK(T):
  STICKY:   group friendly fleets inbound to T by arrival window (width tol). If an EARLIEST
            MATERIAL group exists (group mass >= material_frac * floor — ignore sub-material
            trickle, e.g. a stray 1-ship fleet), D = that group's arrival window. Continue it;
  ...
  FRESH:    else find the tightest feasible wave — the smallest D such that
            { sources with eta(safe_sendable) ≤ D, safe } have total safe_sendable ≥ floor(D).
            D = max source-eta(safe_sendable) in that minimal bundle  (⇒ the FARTHEST needed
            source is the anchor and is launchable now: slack 0). Far launches first, near waits.
  if no feasible D for any candidate ⇒ no wave (target not attackable; see §6 target contract).

DEFENSE(P):
  D_def = earliest grouped deadline D (group enemy arrivals by tol) such that
          relu(floor(P,D) − cover(P,D)) > 0.
...
```
  Pass 1 (own resources only):  remaining0 = relu(floor(P,D_def) − garrison − prod·D_def − friendly_inbound)
        SAFE       if remaining0 ≈ 0
        CANDIDATE  otherwise
  Pass 2 (provisional safe_sendable):  SAFE → garrison − min_reserve into the spare pool;
        CANDIDATE → reserve full garrison (assume it defends, contributes 0 to pool);
        clearly-overwhelmed CANDIDATE (remaining0 ≫ any reachable help) → provisional DOOMED, full garrison to pool.
  Pass 3 (confirm, most-urgent first — ascending D_def, ties by planet id):
        for each CANDIDATE: claim reachable safe mass from the pool (greedy);
        HOLDABLE  if claimable ≥ remaining0 AND hold_value(P) ≥ reinforce_cost   → subtract claimed from pool
        DOOMED    otherwise                                                      → its garrison joins the pool
```
...
ready_sources = sources with ready_now (|slack_to_anchor| ≤ tol)  AND eligible (see decode)
...
later steps arriving at the same `D`). **Bounds overcommit only in continuous pre-rounding
space**; ship-bin rounding (§7) can reintroduce overshoot, which is *logged*, not asserted.

### 3.6 wave_feasible_by_D(T) = total_safe_mass_arrivable_by_D ≥ floor(D)   # attack commit gate
...
```
PPO chain rule unchanged from Phase 4: `p(target)·p(fire|t)·p(ship|t)`.
...
source-target level  (→ fire & ship):
  eta_fast (ETA@safe_sendable)   eta_slow (ETA@min_bin)   ready_now (D_abs in arrival_range)   # §3.0.1
...
ATTACK source s:
  ...
  D  = choose_anchor(t)                         # sticky-material inbound, else tightest-feasible
  send iff:  wave_feasible_by_D(t)              # don't start a doomed wave
             AND ready_now(s, D)                # §3.0.1: D in arrival_range, a legal on-time bin exists
             AND marginal_needed_from_me > 0
  ship_target = min(safe_sendable(s), my_quota)
  chosen_bin  = §3.0.1 ship_target(s): smallest legal bin n with n ≥ quota AND on-time (≤ D+tol).
  ...
  post-round validation: arrival ∈ [D-tol, D+tol] and Σ launched ≥ floor − cover (log misses).
...
I4  each enemy fleet classified exactly once per (T, D)   (no double-count; §3.2)
...
overcommit_ratio (post-round)  bounded   # launched_wave_mass / floor(D)
...
1. `floor` / `cover` / `choose_anchor` / `hold_class` (single-pass, §3.4) / `safe_sendable` /
   ready-wave quota + the ETA contract (§3.0.1) as shared functions + constants. Unit-test each,
   including the ETA↔count round-trip (`ship_target` lands in `[D-tol, D+tol]`).
2. Wave features in both paths + parity test (§10). Update/disable the stale ROI aux
...
3. Direct per-source-target heads; `bc.py` per-target fix (evaluate fire/ship at the teacher's
   chosen target); retire the reinforce overlay / veto per §10.5; wire the `NO_OP` target.
4. Sufficient-prefix **wave** planner (§7) + poisoning checks (§9), incl. the post-rounding
   arrival/floor validation from the ETA contract.
5. Pre-PPO no-veto gate on synthetic states (§11). **Hard stop** until it passes.
6. From-scratch BC → PPO. Promotion gate (§11) before any `wave_head`.
