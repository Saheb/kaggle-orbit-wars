...
Current focus: **🔵 CORRPACK3E LIVE (Jarvis H200 SPOT 217.18.55.72, LR 1e-4, →6M).** Self-anchor re-anchor from corrpack2's 2.5M peak that **WORKED — best-ever Ajay 14.8% + Zach 95.7%** (corrpack3d 524k, full panels; > corrpack2's 11.7% peak, >> old 3.1% record; both high at once). Lineage corrpack3→3b→3c(2.5e-5)→3d(5e-5)→**3e(1e-4)**; resume = IL-ref = corrpack2 2.5M self (anchor UNCHANGED throughout); 512 envs/mb32/workers12, ext 0.6, gate2/floor0/clamp, il-λ0.05. **LR scaling (user call):** clip plateaued ~0.066 at 5e-5 (under-stepping on the 512-env/4096-mb batch) → bumped to 1e-4 at the **1M ckpt boundary** (3e resumes `torch_step_1048576_corrpack3d`). ⚠️ 1e-4 ≈ corrpack2's per-update aggressiveness = the regime it collapsed in (rev38 anchor) — bet = SELF-anchor holds it. **First held-out HOLDS: 3e 524k = 14.5%**, clip→0.09/KL 0.005/il_kl 0.05/EV 0.81/estop 0 = healthy. **Decision rule:** later 3e point <~10% = drift → revert to corrpack3d 524k (14.8%, banked locally) + drop to 5e-5. ⚠️ **HONEST CAVEAT: numeric record NOT structural** — wall INTACT (out-massed 97%, planets@50=6, peel WON 0.60); the 14.8% is competence/board-luck, the force-concentration wall is unbroken (bar is still BEAT rev38 structurally). ⚠️ SPOT → watcher-sync mandatory; `jl destroy` when done. Script `gpu_run_artifacts/corrpack3e/run_remote_corrpack3e_jarvis.sh`; watchers (ajay primary + zach) → `gpu_run_artifacts/corrpack3e/`. **(Predecessors: 3c/3d superseded by the LR bump; corrpack2 collapsed @4M.)**
