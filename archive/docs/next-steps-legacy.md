...
Current focus: **⛔ DECMASS-V2 CONCLUDED — DECOUPLED (the reward-proxy family is now EXHAUSTED). Next: 🟢 CURRICULUM / opponent-relative (make holding-via-concentration NECESSARY to win).** decmass-v2 was the PROPER decmass test (ladder win-gradient + max-ETA + the `dm` leading metrics — all three things decmass1 lacked), run on revedge1 4.72M + `--decisive-mass-coef 0.2`, A100-80GB 512 envs to 2.1M. **VERDICT @2.1M (eval vs Ajay, the TRUSTED metric — read eval not the self-play train `dm` line):** opening `<50 cross` DEAD FLAT 0.26→0.23→0.26, `<50 gap` 0.33 flat; aggregate `cross` ticked 0.54→0.59 but ENTIRELY from late-phase over-kill (`>=100` p50 2.71→3.80, overkill 5.1→6.5 = banking reward on EASY late crossings); **out-massed flat 95%**, Ajay ~24% (≈ resume, noise). = the textbook "farms easy late crossings, leaves the opening under-floor" decoupling (`outmass-limits.md` Part 1 #4), hit the predefined KILL criterion. **Box 428489 destroyed; all spot boxes down.** → reward proxies (decmass/caputil/consolidation/defense_coef/production) are CLOSED. [[project_decisive_mass_lever]]

> **⭐⭐ SESSION VERDICT (2026-06-17) — targeting RULED OUT, sufficiency/sizing confirmed as the lever. Full writeup: `docs/targeting-vs-sufficiency.md`.** Chased the head-audit "fire/target selection is the bottleneck" hypothesis to ground and it does NOT hold: (1) target selection is systematic but **weakly outcome-relevant** — `dom%`/`holdable-ROI best%` separate won/lost by only 4–8pp (winners pick the #1 holdable-ROI target ~32% of the opening and still win); (2) **fire head exonerated** (winner-like `fire_frac` WON 0.18 ≈ Isaiah 0.17; the decmass "floor" is a `coef=0` *reward*, not a training mask); (3) **ROI ablation** (`--ablate-roi`, new) — scrambling roi halved WR (23.8→10.2%) but moved `dm cross` 0.54→0.41 / `cap/atk` 0.59→0.51 with **target metrics FLAT** → roi drives **commitment/conversion, not target choice** (sun placebo `--ablate-sun` ≈ control → roi-specific); (4) real LB loss autopsy (80267509 vs Mike Kim) — sizing **mis-calibrated both ways**: under-commit big rich neutrals (16 vs g25–30) AND over-commit small ones (p50 2.7× vs winner's lean 1.1–1.8×; winner = high cross + lean p50). **NEW tooling:** `decisive-mass NEUTRAL` line (opening land-grab sufficiency), replay winner-vs-loser audit (`audit_replay_head_labels.py`), `near-vs-far`/`holdable-ROI`/`dom%` eval lines, roi/sun ablation flags. → reinforces the existing focus: the lever is **sizing-calibration / sufficiency** (match send to the capture floor, both directions), as a SIGNAL not a reward proxy. [[project_force_concentration_wall]] [[project_decisive_mass_lever]]

> **⭐⭐ SESSION VERDICT (2026-06-16 PM) — the wall, much tighter. Three lever-families RULED OUT this session:**
> - ❌ **Action grammar** — Probe A: we multi-source aggregate at WINNER rates (aggTurn Ajay-WON 0.094 ≥ rank1 0.082; `sameSrc=0`). [[project_aggregation_probe]]
> - ❌ **Target selection / over-extension** — probe_overextension.py: we lose 77-79% of *positionally-SUPPORTED* captures (61-62% of ALL losses were supported); over-extension only +9-11pp. [[project_overextension_probe]]
> - ❌ **Reward proxies** — decmass-v2 confirmed decoupled (above); the whole proxy/outcome reward family is in the graveyard (`docs/outmass-limits.md`).
> **→ THE WALL = concentrating *defense* on defendable captures + *offense* in the opening (`<50` cross 0.23) — contests we are ALREADY positioned for — and it is NOT reward-shapeable.** Indicated next bet = **CURRICULUM / opponent-relative: make holding-via-concentration NECESSARY to win** (boards where a capture is only holdable if you concentrate the defensive response; a contested opening where single-planet probes die so only multi-source survives). Preferred over a defensive hold-floor reward (= the decmass mirror → same decoupling) and over BC/imitation (rev54 crater risk). Full code-grounded reasoning: `docs/outmass-limits.md`.
> **LB harvest in flight:** revedge1 4.72M submitted (sub 53751394, PENDING) — first reinforce ckpt with the reverse-edge cooldown BAKED into export; tests whether the Ajay-panel gains (24%, beats rev38 H2H 81%) translate to LB or confirm the panel≠LB pattern. `docs/submissions.md`.

> **(SUPERSEDED — kept for the baseline) DECMASS-V2 setup:** resume+IL-anchor = revedge1 4.72M; pool = h10/h12/h14 ladder @ ext 0.80; dm BASELINE @ iter1 (self-play train line) gap 0.42/0.33/0.38 · cross 0.21/0.36/0.37. Script `gpu_run_artifacts/decmassv2/run_remote_decmassv2_jarvis.sh` + `deploy_to_box.sh <IP>` (post-preemption redeploy); artifacts `gpu_run_artifacts/decmassv2/`. [[project_force_concentration_wall]]

> **⭐ WHY decmass-v2 (this session 2026-06-16 PM) — caputil killed, Probe A reframed the wall, decmass "negative" downgraded:**
> - **caputil1 (capture follow-through reward) ⛔ KILLED @2.1M.** out-massed flat 95-96%, Ajay 19-21% (below the 22.3% resume), the lever's own target (safe-reinf idle) unmoved at 60%. Triage block showed losses are 98% HOPELESS / 0% cheap-save-missed → we lose captures to being OUT-MASSED, not to bad follow-through; the lever aimed off the wall. Reward conditioned on surviving 30st but median-hold is 13st → can't grip the captures that matter. [[project_capture_utility]]
> - **⭐ Probe A — we ALREADY multi-source aggregate at WINNER rates → action grammar is NOT the wall (skip Probe B).** Measured aggTurn (multi-source→one-target) on revedge1 4.72M vs Ajay/deb, real env, our seat, won/lost split (`orbit_wars_rl/probe_aggregation.py` + `analyze_action_list_replays.py` cherry-picked from spike branch). **Ajay-WON 0.094 / deb-WON 0.078 ≥ rank1 winners 0.082**, up to 4-5 sources/target, **sameSrc=0.000 everywhere** (grammar can't, and we don't need it). Wall = aggregation SUFFICIENCY (size-to-floor) + retention, NOT presence. Won/lost 2.6× gap is mostly phase-composition (~1.3-1.6× phase-matched). RESULTS `gpu_run_artifacts/probe_aggregation/RESULTS.txt`. [[project_aggregation_probe]]
> - **decmass "CONFIRMED NEGATIVE" → INCONCLUSIVE.** decmass1/1b were never fairly tested: deb-only pool (win-starvation), likely mean-ETA floor, and only out-massed% visible (no dm gap/cross). All three fixed now → decmass-v2 is the clean re-test, and Probe A shows it refines SUFFICIENCY on a behavior we already have. [[project_decisive_mass_lever]]

**📄 LIMITS DOC — `docs/outmass-limits.md` (2026-06-16): WHY reward-side fixes keep failing the out-mass wall.** Code-grounded review of the decmass reward's decouplings (un-windowed `mass` = sum-over-ETAs vs combat-resolves-per-arrival; snapshot floor under-models the reactive opponent; per-crossing re-arm is farmable; flat credit flows to EASY crossings not the wall) + the deeper trap (proxy rewards decouple from outcome, outcome rewards get gamed→spray [rev49] or go sparse [caputil/win]). **Read it before designing the next reward term.** Discriminator for decmass-v2: `cross↑` + out-massed flat = reward-design decoupling (fix the reward), not win-starvation. [[project_decisive_mass_lever]] [[project_aggregation_probe]]

**🟡 VDN / per-planet value head — REVISIT *paired with a concentration signal* (not a closed door).** `docs/conclusions.md` shelved per-planet credit (2026-06-09) on the OLD rev53b lineage, judged on general Ajay-WR, BEFORE the wall was diagnosed and the `dm`/out-massed/aggregation metrics existed — and it was never paired with a concentration reward (the doc itself says revisit "with a firing floor"). So the trustworthy conclusion is "per-planet credit ties/loses to joint *on general conversion*", NOT "can't sharpen *sufficiency*". `outmass-limits.md` Part 1 #4 (flat decmass credit → gradient flows to easy crossings, a *spatial* credit-assignment gap) is the candidate new edge-hypothesis. LOW priority vs the reward-grounding fixes; only with a concrete edge hypothesis. VDN code preserved on branch `claude/strange-khorana-6f6141`. [[project_stage1_per_slot_ship]]

> **(PRIOR FOCUS) 🔵 HLADDER (producer-horizon LEAGUE win-gradient; Jarvis H200 SPOT 217.18.55.153, 10M ~4.9hr, machine 428118).** Train ~80% vs a league of **h10 + h12 + h14** + 20% self-play, resume + IL-ref = **3e 4.7M** with the anchor **loosened to il-lambda 0.025**, NO decmass, LR 1e-4, fresh pool (no deb), 512/32/12. **READ:** the EASIEST rung (h10) training-wr climbing past ~40-50% = the win-gradient is finally being caught; then eval `out-massed%` vs Ajay bending below 96%. **TRIPWIRE:** rung-wr↑ but Ajay/Zach held-out (`outmassed_pct` CSV col) ↓ = overfit. ⚠️ SPOT → `jl destroy 428118` when done; balance was ₹890 (~4.7h runway, thin). Script `gpu_run_artifacts/hladder/run_remote_hladder_jarvis.sh`; watchers (ajay+zach) → `gpu_run_artifacts/hladder/`. **⚠️ CORRECTION (2026-06-16): the 3 rungs are sampled UNIFORM (~1/3 each), NOT PFSP-weighted.** With `external_fraction=1.0` the external slice is `r.choice` (uniform); the pool snapshot's `pfsp_w` column is display-only here (it only drives the inactive self-snapshot branch), and self-snapshots show `n=0` (never sampled) — so an earlier "PFSP favors h14 (~half the games)" read was WRONG (it's ~1/3 each). To weight the league toward the matched-difficulty / lose-most rung, relaunch with **`--pfsp-externals`** (opt-in flag, default off, added this session — `opponent_pool._choose_external`). [[project_pool_pfsp_externals]] [[project_h14_wingradient]] [[project_force_concentration_wall]]

**The wall arc this session (2026-06-16) — three negatives converging on one root cause = WIN-STARVATION:**
- **decmass (Lever A — decisive-mass reward) ⛔ CONFIRMED NEGATIVE.** decmass1/1b ran to ~4M: out-massed% dead flat 95-97%, garr@loss→inbound unchanged. The agent optimized the reward (decis 13→19) but NO structural behavior moved → the board-grounded proxy is GAMEABLE (cheap to cross the floor vs weak self-play defenders; symmetric in self-play can't break the under-massing Nash; out-massing is opponent-relative, only learnable by BEATING a forward-projector). USER code-review fixes shipped: P1 mean→max-ETA, stale CLI help, real step()/_auto_reset boundary test. decmass1b killed, **GCP L4 deleted**. [[project_decisive_mass_lever]]
- **corrpack3f (plain-3e continuation) ✅ killed; 3e backfill = TAPPED OUT.** 4.7M→6M Ajay flat 16-18%, out-massed 96-97% throughout → vanilla longer-3e has no headroom on the wall (4.7M is a fine anchor, not a missed peak). [[project_corrpack3_reanchor]]
- **h14only (win-gradient vs ONE winnable planner) ⛔ CONCLUDED NEGATIVE.** Training-wr vs h14 stuck ~25% over 2.1M (NOT the 39.8% the real-env panel showed — the **train/eval SIM-GAP**: torch_env's 144-bin angle quantization makes our agent aim weaker than the fixed-aimer panel, so h14 is HARDER in-sim → still win-starved → out-massed flat). → motivated HLADDER (ladder + PFSP finds the rung winnable IN TRAINING, not just in the panel). [[project_h14_wingradient]]
- **corrpack3e 4.7M SUBMITTED** (sub 53722560, LB pending): best-ever panel (Zach 98.8% / Ajay 18.0%) AND cross-eval validated broad — beats rev38 (LB record) **81% H2H**, 75-94% vs non-planner heuristics (NOT a deb/ajay overfit). [[project_corrpack3_reanchor]]

**🟢 STAGED (next one-delta run, after hladder readout): reverse-edge reinforce cooldown.** Train+eval WIRED + tested (`reinforce_cooldown.py` canonical rule, torch_env `--reverse-edge-cooldown K`, action_mask/build_agent_fn per-game state, ckpt-metadata persist; export deferred until submission). Block own-target B→A reinforce for K steps after A→B (rank1 recip<=3st <0.01 vs our 0.06-0.10). Hygiene, NOT the wall fix; judge by guardrails not the (mask-forced-to-0) recip. [[project_reinforce_pingpong]]

**🔧 DIAGNOSTIC (built 2026-06-16, ALWAYS-ON): decisive-mass GAP (`dm_*`).** Directly measures whether the policy is *moving toward the decmass target* (force concentration) vs only the outcome symptom (`out-massed%`/WR). Per attacked enemy target: `ratio = own_inflight_mass / capture_floor` using the **EXACT reward floor** (shared `torch_env._decisive_mass_fields` → diag and reward can't drift); runs **even at `--decisive-mass-coef 0`** (`--decisive-diag` default ON; per-step P×P pass → benchmark SPS on the GPU box, `--no-decisive-diag` for max-throughput production; uses scatter_reduce-amax → CUDA/CPU fine, crashes local MPS). ⚠️ eval floor defaults to beta 2.2 (not ckpt-stored) — pass `--decisive-mass-beta` to eval to match a non-default-beta run (prints `(beta)` on the line). Train `dm` line + eval `decisive-mass` conversion line (eval adds p50), parity-tested (`tests/test_decisive_mass_diag.py`). **READ:** `dm_gap↓ + dm_cross↑ + overkill steady` = concentrating; **WR↑ but gap flat = adjacent competence** (the decmass1/h14only signature); gap↓ but out-massed flat = attack-concentration without retention. Full field table: `docs/metrics.md` → "The `dm` line". [[project_decisive_mass_lever]] [[project_force_concentration_wall]]
>
> **PRIOR (corrpack3e CONCLUDED @6M — the base for decmass1):** self-anchor re-anchor from corrpack2's 2.5M peak that **WORKED**. Lineage corrpack3→3b→3c(2.5e-5)→3d(5e-5)→3e(1e-4); LR scaled at ckpt boundaries (clip under-stepping at 5e-5 → 1e-4, user call). **3e (1e-4) ran to 6M and CLIMBED — best-ever Ajay 18.0% / Zach 98.8% @ 4.7M** (524k 14.5→1.57M 16.0→3.1M 16.0→4.7M 18.0; no peak-then-fall — 1e-4 held where rev38's anchor didn't in corrpack2). 3d 524k was 14.8%/95.7%. ⚠️ **wall INTACT throughout** (out-massed ~96%, planets@50=6) → the 18% is competence/board-luck, NOT structural (bar is still BEAT rev38 structurally) → exactly what Lever A now targets. Banked locally: 3d 524k+1M, 3e 4.7M.
> ⚠️ **NOTE — corrpack3c carries garrison SOURCE-SELECTION too** (bundled in the box rsync), so it is NOT a pure self-anchor+LR test — source-ranking is a co-delta (changes the action distribution; model is ~permutation-equivariant so low-shock on resume). Accepted by user.
> **⭐⭐⭐ SESSION 2026-06-15 NIGHT (cont.) — SPS external patch + ext-fraction-resume BUG + corrpack3→3b. THE timing breakdown localizes the wall.**
> - **ext-fraction-on-resume BUG (fixed).** `OpponentPool.load()` restores the SAVED `external_fraction`; with `--pool-pinned-fraction 0` the rollout calls `pool.sample(rng)` with no override → **the `--pool-external-fraction` flag is SILENTLY IGNORED on resume.** corrpack3 resumed corrpack2's pool (saved 0.6) and ran at **ext 0.6, not the intended 0.5**, for its whole 524k. Fix: `train_torch` now OVERRIDES the resumed pool's `external_fraction` with the CLI flag (a config knob, not state — members/PFSP stats kept), logging `override external_fraction 0.60 -> 0.50`. ⚠️ **General gotcha: pool CONFIG (external_fraction, and by the same mechanism alpha/max_size) is restored-from-save on resume — only `external_fraction` is now flag-overridden; if you change alpha/max_size on a resumed run, they're still ignored.**
> - **SPS external patch (A+B+C) LANDED + parity-tested** (rides corrpack3b). (A) per-rollout `timing |` diag (pf/ext_obs/ext_wait/ext_conv/estep/upd). (B) `to_legacy_obs_batch` — ONE GPU→CPU copy/field for the external group vs ~8 syncs/env (test_to_legacy_obs_batch: byte-identical incl comets/reordered ids). (C) converter fast path reuses cached `owned_idx/slot_valid` (== owned_indices_for) + one src-pid gather instead of recompute + full `planets.cpu()` (test_heuristic_converter_fastpath: full + subset/reordered parity). Both correctness-preserving (opponent actions never trained on). [[project_sps_pool_optimization]]
> - **⭐ THE TIMING BREAKDOWN (256 envs, deb ext 0.5, 8 workers):** `pf 8.6 ext_obs 2.5 ext_wait 37.7 ext_conv 1.1 estep 9.3 upd 15.0 (74.2s, ext 56%)`. **`ext_wait` = 51% of every rollout = the synchronous deb-worker round-trip = THE wall.** B+C worked — obs-build (3%) + convert (1.5%) are now negligible (the per-env-sync tax we suspected was NOT the dominant cost). SPS 337→379 (~12% from B/C). **The high-value SPS lever is now unambiguous: hide `ext_wait`.**
> - **🟡 PATCH D (async worker overlap) — OPTIONAL, the validated next SPS lever (not built).** External deb obs depends only on the PRE-step state, not our sampled action → submit the deb worker jobs BEFORE the policy forwards, run pf/estep/upd while workers compute, collect results before `env.step()`. Could hide most of the 37.7s `ext_wait` behind the ~33s of pf+estep+upd → potentially ~halve per-rollout time. More invasive (reorders the step loop) than A/B/C; build only if SPS becomes the bottleneck on a long run. Lower-ext or more-workers won't help (latency-, not throughput-bound; 8 workers/16 cores already).
> **⭐⭐⭐⭐ SESSION 2026-06-15 NIGHT — corrpack2 COLLAPSED @4M; relaunched as CORRPACK3 (self-anchor re-anchor) + Producer-v2 integrated + corrpack2 panels.**
> - **corrpack2 COLLAPSE (killed).** Held-out PEAKED @ step 2539520 (2.5M post-resume): **Ajay 11.7% / Zach 94.5%** (Ajay best of the run), then DRIFTED — Zach 95.3→93.8→**80.9%**, Ajay 9.0→8.6→**5.5%** by 4.06M. Internal health stayed CLEAN through the collapse (EV 0.8 / KL 0.005 / clip 0.19 / il_kl 0.45) — classic slow drift the clip can't see, and **the rev38-KL anchor (λ0.05) did NOT catch it.** So 2.54M = the peak; killed the run there.
> - **corrpack2 @ 2.5M PANELS (local, gate2):** vs **rev38_5M H2H = 62.5%** (160/256 — beats our LB-record champion head-to-head) and vs **deb = 11.3%** (29/256, the in-pool training opp). ⚠️ H2H WR is NOT LB-predictive (guardrail only); rev38 doesn't peel so it doesn't test the wall. **Wall fully intact:** vs deb out-massed **97%** (garr@cap 38→@loss 35 vs enemy-inbound 78), planets@50=6, peel WON 0.66; vs rev38 (non-peeler) it HOLDS — peel WON 0.26, planets 2/4/7/9, end 10.4. **Confirms: competent vs RL, the wall is PEELER-specific.** Logs in `gpu_run_artifacts/corrpack2/eval_logs/__rev38_5M.log` / `__debatreya_1300.log`.
> - **CORRPACK3 = re-anchor (LIVE).** Resume + IL-ref BOTH = the corrpack2 2.5M self (self-anchor at our own pre-collapse peak, Stage-A logic; il_kl 0.001 ≈ 0 at start). **DELTAS vs corrpack2:** (1) IL anchor rev38_5M_15g → **2.5M SELF**; (2) **LR 5e-5 → 2.5e-5**; (3) **pool-external-fraction 0.6 → 0.5**; (4) **num-envs 128 → 256** (SPS; mb 32→ kept at **32** = corrpack2-faithful 64 updates/rollout, 2048-sample minibatches — VRAM only 14.5/40GB) + workers 8→4 (16-core oversubscription guard). il-lambda 0.05 CONSTANT (decay_frac 100 → self-anchor persists). **NOTE: the box rsync carried the SPS PACK** (index_features / cached self-model / subset-forward) → corrpack3 runs WITH it (semantics-preserving, validated subset-forward≡full-forward, opponent actions untrained → re-anchor experiment stays clean). **READ:** does held-out Zach/Ajay HOLD at/above the 2.5M peak past 4M (self-anchor damps the drift the rev38 anchor missed) or collapse again (→ cap is pool/reward dynamics, not the anchor).
> - **⚠️ OPS LESSON — Jarvis SPOT preempted MID-RELAUNCH.** Killed corrpack2 on spot box 427274, scp'd+launched corrpack3, then the spot box was **preempted within ~2min** (host key changed, our key rejected, `jl list`=empty). Nothing lost (2.5M ckpt + pool + through-4M all synced locally). Rebuilt on a fresh box. **H100 spot was unavailable** ("no free spot H100"); A100 size is NOT CLI-selectable (`jl create --gpu A100` → 40GB); landed on **A100-40GB ON-DEMAND** (256 envs fits 40GB fine). After killing a run to relaunch on spot, the relaunch box can vanish before it's going — prefer on-demand for the relaunch, or expect to re-provision.
> - **PRODUCER-V2 integrated (`opponents/candidate_producer_v2.py`, gitignored like other candidates).** Pulled `kaggle kernels pull slawekbiel/the-producer-v2` — CLEAN flat agent (not base64-packed), `def agent(obs)` + sibling `orbit_lite` imports (our orbit_lite matches all 13 files). **100% vs random (20/20)** = genuine working planner (clears the ProducerLite 43.8% broken-integration trap). The real **Producer v2 archetype we lacked** (the `β·ρ(eta)·enemy_mass` reachable-enemy-mass term). ⏳ NEXT: strength-screen vs rev38_5M (beatable peeler 30-60% → useful pool opp, vs another 0% wall). Validate-vs-random-FIRST rule held [[feedback_validate_kernel_before_screen]].
> **(prior) CORRPACK2 RUN (Jarvis A100-80GB spot 427274 @ 217.18.55.138, →6M, auto-teardown armed) — COLLAPSED@4M, KILLED (see above).** Resumed corrpack @ **2.5M (torch_step_2539520)** with **correctness pack #2** baked in: per-EPISODE pool attribution (sticky member/seat, raw-winner PFSP crediting, group-by-member override) + time-fraction-weighted rollout diagnostics + obstrunc `enemyship` metric (SSDR+pool/self-boost guarded). NEW run-name because `--resume` resets the step counter to 0 (reusing "corrpack" would collide ckpt step-numbers / held-out CSV). Script `gpu_run_artifacts/corrpack2/run_remote_corrpack2_jarvis.sh`; config else IDENTICAL to corrpack (clamp/gate2/floor0/rev38-KL λ0.05/deb-ext0.6/128envs). ⚠️ SPOT — `jl destroy 427274 --yes` when done. **VALIDATED @ iter7-10 (the 2026-06-15 PM fixes work live):** health clean post-resume (EV 0.70-0.81, KL ~0.006, clip 0.22, estop 0, il_kl ~0.56 anchored); new diag `obstrunc 0.286 (fleet 0.325 ship 0.236 enemyship 0.236)` — **`enemyship == ship` ⇒ ~all hidden fleet mass is the OPPONENT's** (our policy keeps few fleets under cap; deb floods) → **~24% of ENEMY fleet ship-mass is invisible to the model = MATERIAL for the obs256 call** (the decision metric P2b was built for); time-fraction overask/emit-req/satur/reinf compute cleanly; per-episode crediting runs every step error-free (500k pool snapshot = final deb-credited confirmation). ⚠️ **SPS 350→~285 (~18%)** = per-episode multi-member override cost (several `compute_pool_actions` forwards/step vs 1; optimization available: hoist `get_features(seat)` out, shared across self-members). Teardown @5.9M + watchers zach+ajay (from-latest) live on corrpack2.
> **corrpack (pack #1) CONCLUDED — KILLED @2.83M to resume as corrpack2.** Validated the clamp fidelity fixes live (emit/req 0.96-0.99 = launches FIRE vs ~0.42 dropped under old `drop`; satur ~0.01-0.04 low → storage384 NOT needed; obstrunc rose 0.10→~0.45 as deb-flood grew). Held-out zach (gate2): 507904=80.1%, 1523712=82.8%. Clip crept 0.27→0.30 then SETTLED to 0.17 (benign clamp-adaptation, confirmed). Artifacts under `gpu_run_artifacts/corrpack/`.
> **⭐⭐⭐ SESSION 2026-06-15 LATE — SPS PACK: claw back the per-episode-attribution throughput cost (🟢 BUILT+TESTED, ride the NEXT build — do NOT patch corrpack2 mid-run).**
> Per-episode pool attribution is correct but SPS-hostile (350→~285, ~18%): the cost is `compute_pool_actions()` doing, for EACH `(member,seat)` self-group, a full-N `env.get_features(seat)` + a full-N frozen-model forward + a per-group `load_state_dict`, then keeping only its few `env_ids` rows. With several self-members active per step (per-episode = multi-group) that's K× the work. **Fixes (semantics-preserving — opponent actions are never trained on, `train_mask=False`, so changing their sampling RNG alignment is immaterial; verified at the model-OUTPUT level):**
>   1. **Reuse rollout features.** The step loop already computes `get_features(p)` for BOTH seats via `forward_player(p)` → cache into `feats_by_player` and pass to `compute_pool_actions(..., cached_feats=feats_by_player.get(os_))`. Self-groups recompute get_features ZERO times (was K×/step; get_features builds the expensive pairwise tensor).
>   2. **Index BEFORE forward.** New module-level `index_features(feats, ids)` selects the group's envs (handles tensor rows + the length-N `owned_count` list + None) so the frozen model forwards only `len(env_ids)` rows, not all N. Model is LayerNorm-only (no cross-env coupling) → row outputs identical to full-forward-then-index (proven by `tests/test_pool_feature_reuse.py`).
>   3. **Cache frozen self models.** `_get_pool_self_model(member)` caches a tiny deepcopy ON the member (`member._pool_model`) → `load_state_dict` ONCE per member, not per group per step. Lives/dies with the member (GC'd on eviction → no `id()`-reuse hazard; `pool.save()` only serializes named fields so it's never persisted).
>   4. **External path:** the worker-pool branch (deb in the live config) ALREADY builds obs only for `ids` (subset-efficient). DEFERRED: in-process `planner_adapters` (`pa.moves(env_slice=None)` over all envs) — empty in the current lineage (the stalled batched_planner), revisit `env_ids` support only if a planner adapter re-enters the pool.
>   5. **`num_envs=256` sweep:** measure on the GPU box AFTER 1-3 land — larger N may amortize the Python/group overhead further. (No SPS number locally: 8-env CPU/py3.14 toy is throughput-meaningless; the real read is the Jarvis A100.)
> **DECIDED sequencing (user, 2026-06-15):** land items 1-3 AND sweep num_envs 128→256 in the SAME pass (both throughput-only, no learning semantics) on a fresh resume from corrpack2's best ckpt — confirm SPS recovery + clean health BEFORE the next structural delta. Do NOT bundle with PACK 2 (source selector) — keep the infra recovery cleanly attributable.
> **Status:** `tests/test_pool_feature_reuse.py` (subset-forward == full-forward-then-index, non-contiguous/reordered ids; list+None handling) PASS; `test_pool_episode_attribution` + `test_pool_pinned`/`test_pool_ramp` still PASS; 40k-step CPU self-pool smoke ran clean (pool grew to 5 simultaneous self-members = the multi-group case, PFSP credited, no exceptions). **Aim: recover ≥15-25% of the 285→~350 gap with NO material semantics change.** Apply to the build that resumes after corrpack2 plateaus.
> **⭐⭐⭐⭐ SESSION 2026-06-15 LATE — CORRECTNESS PACK #2: POOL EPISODE ATTRIBUTION + obstrunc severity (next-run delta; live corrpack UNTOUCHED).**
> Two finds this session, both pure fidelity ("stop lying to ourselves"), built locally for the NEXT build — the live corrpack run is unaffected and keeps going.
> - **obstrunc SEVERITY diagnostics (built+tested).** The existing `obstrunc` is BINARY (fraction of get_features calls with ANY live fleet past the obs cap F=128) — says some mass is hidden, not HOW MUCH. Added `obs_trunc_fleet_frac` (omitted live fleets ÷ total) + `obs_trunc_ship_frac` (omitted fleet ship-mass ÷ total) + **`obs_trunc_enemy_ship_frac`** (omitted ENEMY fleet ship-mass ÷ total enemy ship-mass — added 2026-06-15 PM per review: total ship_frac mixes our own in-flight reinforcements with the opponent's fleets, so the obs256 DECIDER is the enemy cut, "are we blind to the OPPONENT's mass / an inbound strike?") to the env accumulators (`torch_env.get_features`) + diag line (`obstrunc X (fleet X ship X enemyship X)`). **Decision rule for the parked caps benchmark:** satur <1% → storage384 parked (holds); raise the model's obs fleet cap 128→256 ("obs256") ONLY if the **enemy** omitted mass is MATERIAL, not off the binary rate or total ship_frac alone (a possible further refinement = inbound-to-OWNED enemy mass, needs per-fleet target resolution). **Probe finding (507904):** in SELF-PLAY our policy keeps only ~4-5 live fleets/env → ZERO truncation; so the live run's ~0.45 obstrunc is **the deb opponent flooding the fleet array** (a planner splits sources into many small moves — the action-LIST behavior) → the hidden mass is largely ENEMY fleets, exactly the "is enemy inbound mass hidden?" case. A vs-deb severity probe (owner-split) is the clean read; deb's CPU forward-sim makes it slow to run locally — the metric now rides the next build and will read it live.
> - **⭐ POOL EPISODE ATTRIBUTION fix (BUILT+TESTED — the bigger one).** USER FIND: `pool_opp`/`current_seat` were sampled ONCE PER ROLLOUT (`train_torch`), but env episodes (~300-500 steps) span ~3-4 rollouts and only auto-reset on `done` → a single game could be played by DIFFERENT controllers across rollout boundaries (seat flip / member swap / self-play fallback) and credited to whichever assignment was active at its done step. Harms: (1) **PFSP attribution corruption** — `record_result` feeds the self-pin PFSP weights (deb is `[fixed]` so its EXPOSURE is safe, but the `wr` we read is mis-attributed, not just biased — corrects metrics.md's "don't trust pool wr"); (2) **mid-game controller swaps** inject off-distribution states (current inherits a board built by a different controller / flipped seat); (3) value-bootstrap inconsistency at rollout boundaries. SECONDARY: PFSP win/loss used the SHAPED terminal reward (material/expansion/early-capture all added by then), not the raw winner. **FIX (chosen: per-EPISODE assignment, the correct one):** each env's `(is_pool, member, seat)` is drawn when the env RESETS and held for the whole episode (sticky); the step loop groups pool envs by `(member, opp_seat)` and overrides per-group (`compute_pool_actions` now takes an arbitrary `env_ids` index set, not a contiguous slice); result credited to the env's OWN assignment using a NEW **raw (pre-shaping) winner** `env._last_wins` stashed in `_check_done`. Aggregate pool exposure preserved (each env is pool w/ prob `pool_fraction`); per-rollout opponent DIVERSITY increases (members sampled per-env, not one-per-rollout). Tests: `tests/test_pool_episode_attribution.py` (raw-winner == score winner, independent of win_margin/speed/material shaping) PASS; from-scratch CPU smoke vs an external pool exercises the group-by-member override + crediting + reassignment. ⚠️ Subtlety fixed during build: index_put (`x[ids_t]=y`) does NOT auto-cast dtypes like the old slice copy_ did → cast opp_action/opp_cont to the destination dtype. **Review follow-ups (2026-06-15 PM):** (P1) SSDR (`ssdr_frac>0`) + pool is now GUARDED with a NotImplementedError, NOT ported — the SSDR board asymmetry is applied at the step()-auto-reset using the rollout-start mask, but the per-episode reassignment is drawn AFTER that reset, so a reassigned pool env could inherit an asymmetric board (proper fix = pre-draw next-episode pool-ness for the mask; inert in the current lineage since `ssdr_frac=0`); self-boost (handicap, parked-negative) guarded the same way. (P2a) the rollout diag accumulators (overask/emit-req/satur/reinforce) are now weighted by the train_mask TIME-FRACTION `train_mask.mean(0)` instead of the `t=0` snapshot, so a slot that flips pool/self mid-rollout is attributed ≈correctly (still approximate — env totals aren't split by when they occurred).
>
> **⭐⭐⭐⭐ SESSION 2026-06-15 NIGHT — REINFORCE PING-PONG diagnostic (`reinf_recip`) BUILT + FIRES in held-out → reverse-edge cooldown should JUMP AHEAD of source-ranking.**
> - **USER FIND (replay):** Saheb's reinforce loop is a TEMPORAL ping-pong, NOT same-turn — `A->B` reinforce, arrival, then `B->A` reinforce a couple turns later (e.g. 10↔22 every 2 steps, 185→202→150→162→174→186 ships oscillating). 47 reinforces in one replay, reciprocal within 1/2/3 = 2/9/20. A same-turn role mutex (deb's) does NOT catch this.
> - **METRIC BUILT (eval, `eval._reinf_reciprocity` + conversion line `reinf ping-pong recip<=1/2/3st`):** records each own-target reinforce edge `(step,src,tgt)`; counts reinforces whose NEAREST reverse edge lands within 1/2/3 steps (cumulative, fraction of reinforces). Unit test `tests/test_reinf_reciprocity.py` (incl. the exact replay slice → [0,5,5]). **FIRES live: corrpack2 2.5M vs zach = recip 0.01/0.18/0.20 of 138 reinf** — ~1 in 5 reinforces is a 3-step ping-pong. Rides every panel now (watcher picks it up). ⏳ training-diag version deferred (needs per-env cross-step edge buffers in the vectorized rollout).
> - **🟢 NEXT FIX (jumps ahead of source-ranking per the user's criterion — metric fires in held-out):** **reverse-edge cooldown** — if we launched own-target reinforce A→B recently, BLOCK own-target reinforce B→A for K turns. Do NOT block B *attacking* A (if A turns enemy) or B reinforcing C. Preserves rear-defense + bucket-brigade staging (unlike `forward_only`) while killing this exact waste loop. Build after reading the metric on a deb/ajay panel.
> - **PHASE-STRUCTURED reinforce diagnostics ADDED (eval, from user's rank1/top-agent replay mining).** Top players are NOT "always forward" — there's a phase/size structure: early reinforces forward/outward, mid consolidates toward fight, late logistics freer, **ping-pong rare (<1%)**. New eval lines (per phase <50/50-100/>100, all vs rank1 ref): `reinf by phase fwd-enemy` (nearest-enemy-forward) `[rank1 0.77/0.50/0.48]`, `centroid-out` (mass pushed outward from own centroid) `[0.83/0.69/0.70]`, `target-front-top3` (target ∈ 3 frontline owned) `[0.81/0.34/0.27]`; `reinf fwd-enemy by size <=20/21-50/51+` `[rank1 ≤20:0.42 51-100:0.71]`; ping-pong `recip3 by phase`. **corrpack2 2.5M vs zach read: matches rank1 EARLY (fwd 0.77, front 0.68) but DIVERGES LATE (fwd 0.33, front 0.14) + ping-pong is ~all late (1/9/18)** → confirms early force-projection is fine; the pathology is late logistics. Centroid-out low even early (0.41 vs 0.83) = we keep mass central, a separate gap. → don't hard-mask `forward_only` (blocks legit mid/late logistics); the learning target is "large early/mid reinforces move mass outward/frontline." `tests/test_reinf_reciprocity` extended (phase split).
>
> **⭐⭐⭐⭐ SESSION 2026-06-15 NIGHT — PACK 2: GARRISON-RANKED SOURCE SELECTION (🟢 BUILT+PARITY-TESTED, ready as the NEXT run's delta — NOT on corrpack3b).**
> The "source candidate" pack from the 3-pack plan, framed as source SELECTION (which owned planets occupy the 16 slots), NOT raw planet-id as a feature (ids are arbitrary).
> - **THE SELF-NERF (measured, `/tmp/owned_probe.py` vs zach, corrpack2 2.5M):** all three sites picked the **first 16 owned planets BY ARRAY INDEX**; when >16 owned the rest are inert (can't attack/reinforce/hold). **We own >16 in ~16% of steps (10/16 games), up to 30 owned** — so up to 14 force-bearing planets sat idle. Refuted the "rare" worry decisively.
> - **FIX (garrison-primary, index tiebreak):** the 16 source slots = the highest-GARRISON owned planets. Integer key `-round(ships)*P + idx` (a 1-ship diff = P always outweighs idx<P → exactly "sort by (-ships, idx)"), **parity-exact** across the 3 sites: `VecTorchEnv.owned_indices_for` (topk indices), `features.extract_features` (collect-all → sort → top-16), `action_mask.compute_action_masks` (sort my_planets before slicing). Checkpoint shape-compatible; no-op at <=16 owned.
> - **PARITY TEST** `tests/test_source_selection_parity.py`: train/eval/export agree on top-16-by-garrison **by planet-id-per-slot** (torch indexes the padded tensor, obs paths index the alive-only list — different index spaces, but slot k → same planet; tiebreak orders agree because to_legacy_obs preserves tensor order). Covers >16 owned + an exact garrison tie + no-op under cap. Full regression suite green (feature-parity, masks, pool, friendly-coverage).
> - **SCOPE (honest):** bites in EXPANSION/won games (we snowball to 20–30 vs weaker opp) → LB upside (win-based) + omitted planets can't defend/reinforce (helps HOLD). NOT a direct fix for the planets@50=6 planner wall (we own <16 there). **Effect TBD via a training run.**
> - **DEPLOY:** its own clean run (it changes the action distribution → would confound corrpack3b's re-anchor). Model is ~permutation-equivariant over owned slots (transformer, no owned-slot positional enc) → resuming an index-ranked ckpt should be low-shock (≤16 = pure reorder it's invariant to; >16 = adds high-garrison options), but watch the first ckpts. v2 candidate: fold in frontline/enemy-proximity (needs integer-quantized key to stay parity-safe). [[project_force_concentration_wall]]
>
Prior focus (correctness pack built): A whole-session audit (user, 2026-06-15) found Orbit Wars is an **action-LIST game** (multi-move per turn, multi-move per source, no move/fleet cap — verified vs the installed env AND a live probe: a 10-ship planet emitted 12 one-ship moves from one source) while our policy is a **one-move-per-source, top-16-source** game — the likely core reason planners out-mass/out-coordinate us. Fixes split into 3 sequenced PACKS (don't combine): **(1) CORRECTNESS** (these — fidelity, no new action grammar; DONE, ready to run), **(2) SOURCE CANDIDATE** (ranked top-16 selector replacing "first-16-by-index"; same tensor shapes), **(3) ARCHITECTURE** (multi-move-per-source / raise MAX_OWNED — highest ceiling, but changes the learning problem → cold-start/BC risk). See EVE3 below; EVE2 has the HANDICAP/CONSOL negatives.

> **⭐⭐⭐⭐⭐ SESSION 2026-06-15 EVE3 — SHIP-OVERFLOW TRAIN/EVAL GAP (GAP#2): ~35% of attacks silently DROPPED in training. The top structural lead (read FIRST).**
> Brainstorm after EVE2 (is training-length or ship-binning the blocker?) found a concrete TRAIN/EVAL SIM GAP, the same bug CLASS as the comet gap that was the last real unlock ([[project_train_eval_sim_gap]] GAP#2).
> - **The bug:** when the policy picks a `ship_bin` whose nominal `SHIP_COUNTS[bin]` EXCEEDS the source planet's current garrison — **torch_env TRAINING silently DROPS the whole launch** (`valid_ships = src_ships >= ship_count` → no-op, planet keeps its ships; `torch_env.py:1346`), but **EVAL/kaggle CLAMPS** it to the full garrison (`_ship_bin_to_count = min(SHIP_COUNTS[bin], max_ships)`; `action_mask.py:483`). Training samples ship bins from RAW logits with **NO garrison mask** (`train_torch.py:80-86`) so overflow picks aren't prevented.
> - **MEASURED** (`orbit_wars_rl/ship_commit_probe.py`, argmax/eval-decode, 24g vs deb; gated audit hook `action_mask._SHIP_AUDIT`, default-off): **~35% of ATTACK launches pick bin > garrison → would DROP in training** (consol_6M_final 35.4% over 646 launches; rev38_5M 37.2% over 991; reinforce ~35% too — **LINEAGE-WIDE**, not a consol artifact). Under training's *sampling* (vs eval argmax) likely HIGHER.
> - **TWO IMPLICATIONS:** (1) ~1/3 of the policy's intended full-commit attacks become NO-OPS in training → the gradient that would teach "commit hard, it pays" is MISSING for a third of attempts (signal corruption + train/eval behavior diverges). (2) **REFINES the wall:** the ship head is NOT timid — mean send/garrison ≈ **1.00**, NOTHING below 0.5 garrison; it tries to send ~everything it has. So the out-mass gap is a **multi-planet-AGGREGATION** failure (deb regroups several planets into one strike; we fire from one), NOT small per-launch sends. This corrects the earlier "under-commit" framing.
> - **✅ CORRECTNESS PACK BUILT + TESTED 2026-06-15 (ships in the stageb4-1.5M run; "stop lying to ourselves" fidelity fixes, no new action grammar):**
>   1. **Ship-overflow CLAMP is now the TRAINING DEFAULT** (`--ship-overflow-mode clamp`; `drop`=legacy/A/B-control). `clamp`=`min(ship_count,src_ships)` matches eval's `_ship_bin_to_count`; `drop` silently voided ~35% of attacks in training only (GAP#2). Verified: clamp FIRES, drop VOIDS (realized 80 vs 0). **This supersedes the drop-vs-clamp A/B** — clamp is simply correct, so we ship it (run `drop` explicitly only if you want the legacy baseline).
>   2. **Eval/export 8→16 move cap removed** (GAP#3) — `max_moves=MAX_OWNED_PLANETS` at all 3 action_mask sites; ckpt-compatible.
>   3. **Fleet-debit-on-saturation fixed** (GAP#4) — slot availability computed before debit; ships debited only when a fleet is created (verified: free 0/1/3/5/8 → debit==fleets). No more ship-burn in fleet-full states.
>   4. **Discipline metadata round-trips** — ppo.state_dict persists gate/forward/floor/sufficient-commit; eval+export auto-load (CLI overrides; old ckpts safe). Kills the "omit a mask flag → invalid panel/submission" footgun.
>   5. **Diagnostics on the diag line:** `overask X.XX (<50/50-100/>100 …)` + `emit/req` (emitted fleets ÷ requested launches) + `satur` (fleet_slot_saturation = dropped-no-slot ÷ can_fire) + `obstrunc` (get_features calls with live fleets > obs cap). The saturation + obstrunc numbers FROM OUR OWN ROLLOUTS gate the caps benchmark (storage384 only if we saturate; obs256 only if we truncate). (Also fixed a pre-existing py3.14 argparse `%`-escape bug.)
> - **✅ POST-REVIEW HARDENING (code review, 2026-06-15 — all tested, none touch the live training path):**
>   (a) eval `--reinforce-forward-only` → `BooleanOptionalAction` so `--no-reinforce-forward-only` can override a checkpoint's True;
>   (b) `VecTorchEnv` constructor default flipped `drop`→`clamp` (direct constructors in probes/tests/analysis no longer get the legacy mismatch; `train_torch` already passed clamp);
>   (c) **eval & export now CONSISTENT for OLD reinforce ckpts** — both ERROR requiring an explicit `--reinforce-gate-min-planets` when a ckpt has `allow_reinforce` but no persisted gate (previously eval silently used 0, export baked legacy 2 → a ckpt could eval one way and submit another). New ckpts (this session on) persist the gate → auto-load, no error. ⚠️ Consequence: manually eval/export-ing any PRE-2026-06-15 reinforce ckpt (stageb/consol/handicap/p2rev) now hard-errors without the flag — intended (loud-correct > silent-wrong); the watchers already pass it;
>   (d) committed regression test `orbit_wars_rl/tests/test_fleet_saturation_burn.py` (asserts debit==fleets_created at free slots 0/1/3/5/8);
>   (e) doc/memory contradiction fixed (GAP#2 now reads FIXED everywhere).
> - **⚠️ Caveat:** the correctness pack improves train/eval fidelity but does NOT by itself teach multi-planet aggregation or multi-move/source (the action-LIST ceiling) — those are PACKS 2 (ranked source selector) + 3 (architecture), deliberately deferred so the resume ckpt isn't shocked by a moving interface. Caps = the separate measure-first benchmark (GAP#4 doc block in train-eval.md).
> - **⏸ PARKED ARCH CEILING (logged 2026-06-15, NOT in the current A/B — deferred by user):** the policy can emit only **ONE move per source planet per turn** (action interface = one slot per owned planet, `MAX_OWNED=16`, each slot → one `[fire,target/angle,ship_bin]`). The OFFICIAL env allows a source to appear in MULTIPLE moves the same turn — `orbit_wars.py:process_moves` deducts ships per move sequentially (`from_planet[5] >= ships` then `-= ships`), so a source can split its garrison into several fleets to different targets/angles in one turn. Strong planners exploit this (Suneet-style `spent_total[src_id]` budgeting). Caps our **fan-out / per-turn tempo** — the *split-one-source-to-many* axis (complementary to the concentration wall's *aggregate-many-to-one*). Likely a MAJOR ceiling vs planner-like agents. Fix = a real action-space redesign (multi-move-per-source / autoregressive or per-source budget head) → deferred past the cheap A/B fixes. Ties to lever C.
> - **Other brainstorm verdicts:** (Q-scale) training 100M-600M like the field is **mostly false hope ON OUR SETUP** — out-massed is FLAT not crawling across 5M→13M, self-play Nash doesn't price concentration, and we're win-starved vs the only opponent that'd punish it; scale multiplies a CORRECT setup, won't rescue the wrong equilibrium (+ ~70-420h/run on spot kills iteration). (Q-bins) ship bins are NOT a hard cap (table maxes 420, multi-planet sends are small) — the issue was the DROP rule above, not granularity. `ship_bin_mode="fraction"` (10 bins = %-of-garrison, never drops) is a related candidate (D) that also removes the gap.
>
