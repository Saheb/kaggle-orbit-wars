...
Current focus: **🔵 CORRPACK3 LIVE (Jarvis A100-40GB ON-DEMAND 427628 @ 217.18.55.72, →6M).** Re-anchor from corrpack2's 2.5M PEAK to escape the 4M collapse. ⚠️ ON-DEMAND bills continuously, NO auto-teardown → `jl destroy 427628 --yes` when done. Script `gpu_run_artifacts/corrpack3/run_remote_corrpack3_jarvis.sh`; watchers (zach + ajay from-latest) live → `gpu_run_artifacts/corrpack3/`.
