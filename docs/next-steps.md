...
Current focus: **🔵 HLADDER LIVE (producer-horizon LEAGUE win-gradient; Jarvis H200 SPOT 217.18.55.153, 10M ~4.9hr, machine 428118).** Train ~80% vs a PFSP league of **h10 + h12 + h14** + 20% self-play, resume + IL-ref = **3e 4.7M** with the anchor **loosened to il-lambda 0.025**, NO decmass, LR 1e-4, fresh pool (no deb), 512/32/12. **READ:** the EASIEST rung (h10) training-wr climbing past ~40-50% = the win-gradient is finally being caught; then eval `out-massed%` vs Ajay bending below 96%. **TRIPWIRE:** rung-wr↑ but Ajay/Zach held-out (`outmassed_pct` CSV col) ↓ = overfit. ⚠️ SPOT → `jl destroy 428118` when done; balance was ₹890 (~4.7h runway, thin). Script `gpu_run_artifacts/hladder/run_remote_hladder_jarvis.sh`; watchers (ajay+zach) → `gpu_run_artifacts/hladder/`. [[project_h14_wingradient]] [[project_force_concentration_wall]]
