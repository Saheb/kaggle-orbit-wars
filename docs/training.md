...
**Diagnostics shipped this session (all eval-only unless noted; see `docs/metrics.md`):** `dm_*` decisive-mass GAP (train+eval, `--decisive-diag` default-on — are inflight attacks reaching producer_v2's capture floor); `hold-floor` (defensive mirror, by phase + age-after-capture); `outmassed by planets@32` (computed verdict — **2026-06-16 Ajay finding: opening expansion NOT the wall lever**, >=6 still 97% out-massed); `game-len` (median split by outcome — stall-and-win vs decisive); `reinforce-triage` (save-efficiency vs producer_v2's safe_drain+roi — the **bad-triage hunch**, [[project_reinforce_triage]]); `--pfsp-externals` pool flag ([[project_pool_pfsp_externals]]).
