...
Current focus: **🔵 DECMASS1 LIVE (Lever A — decisive-mass reward; Jarvis H200 SPOT 217.18.55.72, →6M ~2.2hr).** FIRST structural attack on the force-concentration wall (scalar reward/pool knobs all failed — HANDICAP/CONSOL negative). Rewards our INFLIGHT force converging on an ENEMY planet the step it first reaches **producer_v2's capture_floor** (`garrison + prod·eta + enemy_inbound + beta·ρ(eta)·reachable_enemy_mass + 1`; beta2.2/eta_free3/eta_scale12/horizon18 = v2-faithful). Board-grounded, NOT outcome-tied → injects the concentration gradient self-play can't price; reward-side only → NO eval/obs parity risk. **USER FIND:** the v1 floor `ships+prod·3+1` was a static proxy ~HALF the real value (missed the reactive `beta·ρ·enemy_mass` margin = the not-yet-launched defense that out-masses us). Commit `8ff1eb7`; `--decisive-mass-coef 0.2 --decisive-mass-beta 2.2` (lower beta to ~1.0 if `decis`~0); test_decisive_mass. Resume = IL-ref = **corrpack3e 4.7M (Ajay 18.0% / Zach 98.8% — new best-ever)** = self-re-anchor at our best (il_kl 0.005 = low-shock) + LR 1e-4; ONE real delta = the lever. **iter1: decis 11.98** (fires strong on the trained policy), clip 0.108/KL 0.013/estop 0 healthy. **READ:** eval `out-massed%` DOWN from ~96% + `garr@loss→enemy-inbound` (31 vs 71) CLOSING; Ajay holds/climbs from 18%. **KILL: flat out-massed% by ~1.5M ⇒ blocker is ARCHITECTURAL → lever C** (multi-move-per-source). ⚠️ SPOT → watcher-sync mandatory; `jl destroy` when done. Script `gpu_run_artifacts/decmass1/run_remote_decmass1_jarvis.sh`; watchers (ajay+zach) → `gpu_run_artifacts/decmass1/`. See [[project_decisive_mass_lever]] [[project_force_concentration_wall]].
>
