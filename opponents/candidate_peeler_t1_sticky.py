...
class ProducerLiteConfig:
    """Peeler t1 sticky: medium expansion, extra pressure on thin enemy holds."""
    ...
    max_offensive_targets: int = 6          # enemy/neutral proximity targets
    max_defensive_targets: int = 4          
    # --- scoring / greedy ------------------------------------------------
    max_waves_per_turn: int = 3
    roi_threshold: float = 2.2              # fire if score > this
    min_ships_to_launch: float = 6.0
    # Retake pressure: boost enemy-owned targets that look thin relative to
    # their production. This keeps the opponent beatable on expansion while
    # making newly captured, under-garrisoned frontier planets more salient.
    sticky_thin_bonus: float = 1.4
    sticky_thin_ship_scale: float = 35.0
    sticky_prod_weight: float = 0.35
...
def plan_lite_waves(
    *,
    ...
    if float(config.sticky_thin_bonus) > 0.0:
        tgt = target_idx[cand_tgt_short].clamp(0, P - 1)
        target_owner = obs.owner_abs[tgt]
        enemy_owned = (target_owner >= 0) & (target_owner != pid)
        target_ships = obs.ships[tgt].to(dtype)
        target_prod = prod[tgt].to(dtype)
        thin = (1.0 - target_ships / float(config.sticky_thin_ship_scale)).clamp(min=0.0, max=1.0)
        prod_term = target_prod / (target_prod + float(config.sticky_prod_weight))
        sticky_bonus = float(config.sticky_thin_bonus) * thin * prod_term
        score = score + torch.where(enemy_owned, sticky_bonus, torch.zeros_like(sticky_bonus))
