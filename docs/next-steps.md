...
Current focus: **🔵 CORRPACK3C LIVE (Jarvis H200 SPOT 427683 @ 217.18.55.72, →6M, ~780 SPS → ~2hr).** Clean self-anchor re-anchor from corrpack2's 2.5M PEAK (escape the 4M collapse). Resume = IL-ref = corrpack2 2.5M self; **only learning deltas vs corrpack2 = self-anchor (rev38→2.5M-self) + LR 5e-5→2.5e-5**; ext back to 0.6, 512 envs/mb32/12 workers; carries SPS patch + garrison source-selection + reinf-recip + phase diagnostics. ⚠️ SPOT (preempted twice already) → watcher-sync mandatory; `jl destroy 427683 --yes` when done. Script `gpu_run_artifacts/corrpack3c/run_remote_corrpack3c_jarvis.sh`; watchers (zach + ajay from-latest) → `gpu_run_artifacts/corrpack3c/`. READ: held-out holds at/above the 2.5M peak past 4M. **(Predecessors: corrpack3 ran wrong ext 0.6 [resume-bug], corrpack3b fixed ext→0.5 but its A100 box HUNG @~1M — both superseded.)**
