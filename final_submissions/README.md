...
| `presres1_0.5M_raw.pt` | Same training step, raw save without the backfilled flag (kept for provenance). |
...
- The .pt files require the pre-cleanup code to load/export (feature semantics: resolver ON,
  gate2/floor0/no-forward, reverse-edge cd3, game-phase 15-global, target-decode). If the
  cleaned code drifts, load them from the pre-cleanup git tag.
