...
def test_proactive_saves_keeps_unthreatened_but_still_drops_late_hopeless_gate():
    stats = Counter()
    # Unthreatened (proactive forward-staging) save is KEPT under --proactive-saves,
    # where the reactive filter drops it as no-threat.
    assert _passes_filters(_save_label(enemy_fleet=False), _args(proactive_saves=True), stats)
    assert stats["kept_proactive_save_moves"] == 1
    assert stats["filtered_save_no_threat"] == 0

    # Hopeless (threatened, source can't cover) is STILL dropped.
    assert not _passes_filters(_save_label(source_garrison=1), _args(proactive_saves=True), stats)
    assert stats["filtered_save_hopeless"] == 1

    # Gate discipline is retained.
    assert not _passes_filters(_save_label(owned_count=1), _args(proactive_saves=True), stats)
    assert stats["filtered_save_gate"] == 1
    print("test_proactive_saves_keeps_unthreatened_but_still_drops_late_hopeless_gate: PASS")
