...
class ProducerLiteConfig:
    """Peeler t0: weak expansion, preserved reactive peel."""

    
    # the projection window, the movement build length, AND the target ETA cap 
    horizon: int = 10
    # --- shortlists ------------------------------------------------------
    max_sources_per_lane: int = 8
    max_offensive_targets: int = 4          # enemy/neutral proximity targets
    max_defensive_targets: int = 4          
    # --- scoring / greedy ------------------------------------------------
    max_waves_per_turn: int = 2
    roi_threshold: float = 3.0              # fire if score > this
    min_ships_to_launch: float = 8.0
