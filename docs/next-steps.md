...
Current focus: **🔵 CORRPACK3B LIVE (Jarvis A100-40GB ON-DEMAND 427628 @ 217.18.55.72, →6M).** Self-anchor re-anchor from corrpack2's 2.5M PEAK (escape the 4M collapse), continued from corrpack3's 524k ckpt. ⚠️ ON-DEMAND bills continuously, NO auto-teardown → `jl destroy 427628 --yes` when done. Script `gpu_run_artifacts/corrpack3b/run_remote_corrpack3b_jarvis.sh`; watchers (zach + ajay from-latest) live → `gpu_run_artifacts/corrpack3b/`.
