...
set -uo pipefail
# Auto-derive ROOT from this script's location so each worktree's watcher evaluates with its
# OWN code (orbit-audit's eval.py != kaggle's after the phase5 no_op_head split). Resolves to
# the same path the hardcode had for the kaggle worktree, so no behaviour change there.
