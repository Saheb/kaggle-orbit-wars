...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--game-phase-features", action="store_true", default=False,
                    help="Emit the 15-global game-phase feature vector (matches bc.py "
                         "--game-phase-features and the PPO model config). Default off = 11-global.")
...
def main() -> None:
    ...
    if args.game_phase_features:
        # The build inserts both repo-root and orbit_wars_rl/ on sys.path, so `features` (used by
        # bc.trajectory_to_training_sample, which builds the SAVED sample) and `orbit_wars_rl.features`
        # (used here in _context) are SEPARATE module objects. Flag BOTH so every extract_features
        # call emits the 15-global game-phase vector — else the saved globals silently stay 11.
        import features as _features_top
        from orbit_wars_rl import features as _features_pkg
        _features_top.set_game_phase_features(True)
        _features_pkg.set_game_phase_features(True)
