...
- **Pricing model:** **on-demand always** (never spot). Spot interruptions in
  us-east-1 for g5.2xlarge are frequent enough to lose mid-run checkpoints
  (4 interrupts in one session, avg runtime <2h before reclaim). On-demand cost
  for a ~12h run is ~$15 — acceptable. Always use `launch_gpu.sh` which sets
